...
function UploadFile(strFileName, strPath, uploadPath, strFieldName, OnSuccess, OnFail) {
    ...
    const params = uploadPath.substring(uploadPath.indexOf('?'),uploadPath.length).split('&');
    let readOnlyFlag = false;
    for(let i=0;i<params.length;i++){
      // caocao 从请求参数中获取token
      if(params[i].indexOf('token')>=0){
        const tokenArr = params[i].split('=');
        const token = tokenArr[1];
        xhr.setRequestHeader("Authorization", token);
        continue;
      }
      //caocao 从请求参数中获取是否只读 只读模式不让保存到服务器
      if(params[i].indexOf('readOnly')>=0){
        const tokenArr = params[i].split('=');
        const readOnly = tokenArr[1];
        if(readOnly=='true'){
          readOnlyFlag = true;
          break;
        }
