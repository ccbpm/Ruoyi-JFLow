...
function NodeFrmD(nodeID) {
    ...
    if (node.FormType == 1 || node.FormType == 0 || node.FormType == 10) { //自由表单，经典表单.
        //经典表单.
...
$(function () {
    ...
    var alertModal5 = $('#alertModal5'), attributeModal = $("#attributeModal");
    //消息提示
    ...
    //新建节点消息提示
    newNodeAlert = function (messages, s) {
        if (!messages) messages = "";
        if (!s) s = 2000;
        alertModal5.find(".modal-body").html(messages);
        alertModal5.modal('toggle');
        setTimeout(function () { alertModal5.modal("hide") }, s);
    }
    //消息弹出（节点）
...
//新建节点提示
function NewNodeHelp() {

    var msg = "<ul>";
    msg += "<li> <img src='../../Img/newNodeMsg.png' /></li>";
    msg += "<li>对画布进行右击即可进行新节点创建.</li>";
    msg += "</ul>";
    newNodeAlert(msg, 20000);
    return;
}
