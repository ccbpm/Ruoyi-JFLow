...
namespace CCFlow.WF.CCForm
{
    ...
    public class CCFormAPI : System.Web.Services.WebService
    {
            ...
            ref DataSet ds, ref byte[] bytes)
        {
            ...
            //是否可以查看该工作.
            bool b = BP.WF.Dev2Interface.Flow_IsCanViewTruck(gwf.FlowNo, gwf.WorkID);
            if (b == false && 1 == 2)
                throw new Exception("err@[" + userNo + "]无权查看该流程,WorkID=" + workID);

            string frmID = "ND" + int.Parse(gwf.FlowNo) + "Rpt";
            BP.WF.GERpt rpt = new BP.WF.GERpt("ND" + int.Parse(gwf.FlowNo) + "Rpt", workID);
            ...
            //把从表数据加入里面去.
            MapDtls dtls = new MapDtls("ND" + gwf.FlowNo);
            foreach (MapDtl item in dtls)
            {
                GEDtls dtlEns = new GEDtls(item.No);
                dtlEns.Retrieve(GEDtlAttr.RefPK, workID);
            ...
            //生成模版的文件流.
            BP.WF.Template.Frm.FrmPrintTemplate template = new BP.WF.Template.Frm.FrmPrintTemplate(billTemplateNo);
        ...
        public void WordFileGener(string userNo, string sid, Int64 workID, ref byte[] bytes)
        {
            ...
            BP.WF.Template.Frm.MapFrmWord mfe = new BP.WF.Template.Frm.MapFrmWord(frmID);
        ...
        public bool GenerExcelFile(string userNo, string sid, string frmID, string pkValue, ref byte[] bytes)
        {
            ...
            if (frmID.Contains("BP."))
            {
                ...
                //return md.ExcelGenerFile(ref bytes);
            }
            else
            {
                MapData md = new MapData(frmID);
        ...
        /// <param name="sid">SID</param>
        /// <param name="frmID">表单ID</param>
        /// <param name="pkValue">表单主键</param>
        /// <param name="bytes">二进制数组</param>
        /// <returns></returns>
        [WebMethod]
        public bool GenerTemplateFile(string userNo, string sid, string frmID, string pkValue, ref byte[] bytes)
        {
            ...
            if (frmID.Contains("BP."))
            {
                ...
                MapData md = en.DTSMapToSys_MapData();
        ...
        public void SaveExcelFile(string userNo, string sid, string frmID, string pkValue, string mainTableAtParas, System.Data.DataSet dsDtlsChange, System.Data.DataSet dsDtlsOld, byte[] byt)
        {
            ...
            Log.DebugWriteInfo("写入VSTO表单从表数据：" + BP.DA.DataTableConvertJson.Dataset2Json(dsDtlsChange));
            ...
            if (dsDtlsChange != null)
            {
                ...
                foreach (System.Data.DataTable dt in dsDtlsChange.Tables)
                {
                    foreach (MapDtl dtl in dtls)
                    {
                        ...
                        if (dtl.ItIsReadonly) //从表是否只读
                            continue;

                        #region 执行删除操作.
                        if (dtl.ItIsDelete == true) //从表是否可删除行
                        {
                        ...
                        #region 执行更新操作.
                        if (dtl.ItIsUpdate == true) //从表【是否可更新行】
                        {
                            //获取dtls
                            ...
                            foreach (DataRow dr in dt.Rows)
                            {
                                ...
                                if (fes.Count > 0)
                                {
                                    string msg = fes.DoEventNode("RowSaveBefore", mainEn);
                                    if (msg != null)
                                ...
                                if (febd != null)
                                {
                                    febd.HisEn = mainEn;
                                    febd.HisEnDtl = daDtl;
                                    febd.DoIt("RowSaveBefore", febd.HisEn, daDtl, null);
                                }
                                ...
                                if (daDtl.OID <= 100 && dtl.ItIsInsert == true) //从表【是否可新增行】.
                                {
                                    if (daDtl.ItIsBlank == false)
                                    {
                                        daDtl.RefPK = pkValue;
                                ...
                                if (fes.Count > 0)
                                {
                                    string msg = fes.DoEventNode("RowSaveAfter", daDtl);
                                    if (msg != null)
                                        throw new Exception(msg);
                                }
                                ...
                                if (febd != null)
                                {
                                    ...
                                    febd.DoIt("RowSaveAfter", mainEn, daDtl, null);
                                }
            ...
            //求出从表实体类.
            BP.Sys.Base.FormEventBase frmEvent = null;
            if (md.FormEventEntity != "")
                frmEvent = BP.Sys.Base.Glo.GetFormEventBaseByEnName(md.No);
            ...
            if (pkType == "MyPK")
            {
                ...
                //保存前(执行类事件.)
                if (frmEvent != null)
                    frmEvent.DoIt("SaveAfter", wk, null);
            }
