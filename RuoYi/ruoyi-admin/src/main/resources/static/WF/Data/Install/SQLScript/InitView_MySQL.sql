...
Title,RDT,ADT,SDT,FK_Emp,FID,FK_FlowSort,SysType,SDTOfNode,SendDT,GuestNo,GuestName,BillNo,
TodoEmps,TodoEmpsNum,TodoSta,TaskSta,FlowNote,ListType,Sender,AtPara,DomainExt,OrgNo,FlowIdx,FlowSortIdx )
AS
...
B.SDT, B.FK_Emp,B.FID ,A.FK_FlowSort,A.SysType,A.SDTOfNode,A.SendDT,
A.GuestNo,A.GuestName,A.BillNo,A.TodoEmps,A.TodoEmpsNum,A.TodoSta,A.TaskSta,A.FlowNote,0 as ListType,A.Sender,A.AtPara,
A.DomainExt,A.OrgNo,C.Idx AS FlowIdx, D.Idx AS FlowSortIdx
FROM  WF_GenerWorkFlow A, WF_GenerWorkerlist B,WF_Flow C,WF_FlowSort D
WHERE     (B.IsEnable = 1) AND (B.IsPass = 0)
 AND A.WorkID = B.WorkID AND A.FK_Node = B.FK_Node AND A.WFState!=0  AND WhoExeIt!=1 AND A.FK_Flow=C.No AND A.FK_FlowSort=D.No AND ((C.FK_FlowSort = '' ) OR ( C.FK_FlowSort = D.No ))
 UNION
...
A.PFlowNo,B.NodeIDCC as FK_Node, B.NodeIDCCName AS NodeName, A.Title, A.RDT, B.RDT AS ADT, 
B.RDT AS SDT, B.CCTo as FK_Emp,B.FID ,A.FK_FlowSort,A.SysType,A.SDTOfNode,A.SendDT,
...
-- 对象:  View WF_Todolist    脚本日期:  2024-08-16;
CREATE VIEW WF_Todolist
AS 
SELECT CONCAT(A.WorkID, '_', B.FK_Emp) AS MyPK, A.WorkID AS WorkID, A.WorkID AS OID, A.Title, A.StarterName,A.RDT, A.NodeName ,A.TodoEmps ,A.SendDT,A.Sender,
A.FK_Flow AS FlowNo, A.FlowName, B.FK_Emp AS EmpNo, A.OrgNo
FROM WF_GenerWorkflow A, WF_GenerWorkerlist B 
WHERE A.WorkID =B.WorkID AND A.FK_Node =B.FK_Node AND B.IsEnable =1 AND B.IsPass =0