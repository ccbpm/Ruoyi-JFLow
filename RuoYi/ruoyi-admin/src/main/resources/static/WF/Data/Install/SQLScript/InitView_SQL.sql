...
RDT,ADT,SDT,FK_Emp,FID,FK_FlowSort,SysType,SDTOfNode,SendDT,
GuestNo,GuestName,BillNo,TodoEmps,TodoEmpsNum,TodoSta,TaskSta,FlowNote,
ListType,Sender,AtPara,DomainExt,OrgNo,FlowIdx,FlowSortIdx)
AS
...
A.FK_Dept,A.DeptName, A.FK_Flow, A.FlowName,A.PWorkID,
A.PFlowNo,
B.NodeIDCC FK_Node, B.NodeIDCCName AS NodeName,   A.Title, A.RDT, B.RDT AS ADT, 
...
'@IsCC=1'+'@NodeIDWork='+convert(varchar(100),B.NodeIDWork)+A.AtPara as AtPara,
A.DomainExt,A.OrgNo,C.Idx AS FlowIdx, D.Idx AS FlowSortIdx
...
-- 对象:  View V_FlowStarterBPM 脚本日期:  2015-04-10 . V_FlowStarterBPM -- 按绑定岗位.-- 按绑定部门的人员.-- 按绑定人员的人员.;
...
CREATE VIEW WF_Todolist (MyPK,WorkID,OID,Title,StarterName,RDT,NodeName,TodoEmps,SendDT,Sender,FlowNo,FlowName,EmpNo,OrgNo)
AS
SELECT
        A.WorkID+ '-'+ B.FK_Emp AS MyPK,
		A.WorkID AS WorkID,
        A.WorkID AS OID,
        A.Title AS Title,
