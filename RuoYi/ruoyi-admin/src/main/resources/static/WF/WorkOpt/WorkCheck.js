...
var checkParam = {
    FK_Flow: GetQueryString("FK_Flow") || GetQueryString("FlowNo") ,
    FK_Node: GetQueryString("FK_Node")  || GetQueryString("NodeID") ,
    ...
    if (tracks.length != 0) {
        ...
        $.each(tracks, function (idx, item) {
            if (frmWorkCheck.SigantureEnabel == 3 || frmWorkCheck.SigantureEnabel == 4 || frmWorkCheck.SigantureEnabel == 5)
                _Html += WorkCheck_Stamp_Parse(item, frmWorkCheck, 1, true, FWCVer, idx,0);
            else
                _Html += WorkCheck_Parse(item, frmWorkCheck, SignType, 1, true, FWCVer,0);
...
//签批组件专用方法.
function GetWorkCheck_Node(checkData, keyOfEn, checkField, FWCVer,isMobile) {
    //当前节点审核组件信息
    frmWorkCheck = checkData.WF_FrmWorkCheck[0];
 ...
 */
    ...
    for (var i = 0; i < tracksArr.length; i++) {
        ...
        if ($("#TB_" + keyOfEn).length != 0 && $("#TB_" + keyOfEn).val().indexOf("," + track.NodeID) == -1)
            continue;
        if (frmWorkCheck.SigantureEnabel == 3 || frmWorkCheck.SigantureEnabel == 4 || frmWorkCheck.SigantureEnabel == 5)
            _Html += WorkCheck_Stamp_Parse(track, frmWorkCheck, 0, isShowCheck, FWCVer, i, isMobile);
        else
            _Html += WorkCheck_Parse(track, frmWorkCheck, SignType, 0, isShowCheck, FWCVer, isMobile);
    }
...
function WorkCheck_Parse(track, frmWorkCheck, SignType, showNodeName, isShowCheck, FWCVer,isMobile) {
    ...
    if (SignType == null || SignType == undefined) {
        ...
        if (isEditWorkCheck == true && getConfigByKey("IsShowWorkCheckUsefulExpres", true) == true && isMobile == 0) {
        ...
        }else if (isEditWorkCheck == true && getConfigByKey("IsShowWorkCheckUsefulExpres", true) == true && isMobile == 1) {
            //加入常用短语.
            var fwcs = new Entities("BP.WF.Template.FrmWorkChecks");
            fwcs.Retrieve("NodeID", track.NodeID);
            //获取自定义常用短语
            //debugger
            ...
            _Html += "<select id='DuanYu' onchange='SetDocVal();SaveWorkCheck();' style='border:1px solid black;padding: 0px;margin: 0px;'>";
        ...
        if (rdt == "") {
            ...
            rdt = dt.getFullYear() + "-" + month + "-" + dt.getDate() + " " + dt.getHours() + ":" + dt.getMinutes();  // new Date().toString("yyyy-MM-dd HH:mm");
        }
        if (isEditWorkCheck == true && frmWorkCheck.IsChangeFWCTime=='1' )
            _Html += "<div class='layui-col-sm3 layui-col-md3 layui-col-xs4 ' style='float: right; margin-left: 10px;'><div class='ccbpm-input-group'><input class='ccdate layui-input'  data-info='yyyy-MM-dd HH:mm' data-type='datetime'  value='" + rdt + "'  type='text' id='WorkCheck_RDT' style='display:inline; width:auto' /><i class='input-icon layui-icon layui-icon-date'></i></div></div>";
        else
            _Html += "(" + rdt + ")";
        _Html += "</div>";
...
function WorkCheck_Stamp_Parse(track, frmWorkCheck, showNodeName, isShowCheck, FWCVer, idx, isMobile) {
...
function SaveWorkCheck(type) {
    ...
    if ($("#WorkCheck_RDT").length == 1)
        handler.AddPara("CheckTime", $("#WorkCheck_RDT").val());
