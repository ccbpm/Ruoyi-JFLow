...
public abstract class Entity extends EnObj implements Serializable
{
	...
	public final DataTable ToDataTableField(String tableName)  {
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			...
			else
			{
				...
				{
					dr.setValue(attr.getKey(), (Object)null);
					continue;
				}
	...
	*/
	...
	public  DataTable RunSQLReturnTable(String sql, Paras paras) throws Exception {
		return this.getHisDBSrc().RunSQLReturnTable(sql,paras);
		/*switch (this.getEnMap().getEnDBUrl().getDBUrlType())
		{
			case AppCenterDSN:
	...
	*/
	...
	public final int RunSQLReturnValInt(String sql, int IsNullReturnVal) {
		Object obj = "";
	...
	public final int RunSQLReturnValInt(String sql, Paras paras) throws Exception {
		...
		SFDBSrc sfdbSrc = this.getHisDBSrc();

		paras.SQL=sql;
		switch (this.getEnMap().getEnDBUrl().getDBUrlType())
		{
			case AppCenterDSN:
				return sfdbSrc.RunSQLReturnValInt(paras.SQL,paras, 0);
	...
	*/
	...
	public final String GenerNewNoByKey(String attrKey, Attr attr) throws Exception {
		try
		{
			...
			String str = String.valueOf(this.getHisDBSrc().RunSQLReturnValLong(sql, 1));
	...
	*/
	public final String GenerNewNoByKey(int nolength, String attrKey, String attrGroupKey, String attrGroupVal) throws Exception {
		...
		DataTable dt = this.getHisDBSrc().RunSQLReturnTable(sql, ps);
		String str = "1";
		if (dt.Rows.size() != 0)
		{
	...
	*/
	public int DirectInsert() throws Exception
	{
		try
		{
			...
			{
				case MSSQL:
					return sfdbSrc.RunSQL(this.getSQLCache().Insert, SqlBuilder.GenerParas(this, null));
				case Access:
					return sfdbSrc.RunSQL(this.getSQLCache().Insert, SqlBuilder.GenerParas(this, null));
				case MySQL:
				case Informix:
				default:
					return sfdbSrc.RunSQL(this.getSQLCache().Insert.replace("[", "").replace("]", ""), SqlBuilder.GenerParas(this, null));
			}
		}
		catch (RuntimeException ex)
		{
...
//        }
		...
		try {
			...
			if (msg.contains("does not exist")
					|| (msg.contains("不存在") && msg.contains("记录[枚举注册  Sys_EnumMain,") == false)
					|| msg.contains("doesn't exist") //@wwh.
					|| msg.contains("无效")
	...
	*/
	public final int Delete(Object pk) throws Exception
	{
		...
		switch (this.getEnDBType())
		{
			...
			case GBASE8CByMySQL:
			case GBASE8A:
				return sfdbSrc.RunSQL("DELETE FROM " + this.getEnMap().getPhysicsTable() + " WHERE " + this.getPK() + " =" + this.getHisDBVarStr() + pk);
			default:
	...
	*/
	public final int Delete(String attr, Object val) throws Exception
	{
		...
		return this.getHisDBSrc().RunSQL("DELETE FROM " + this.getEnMap().getPhysicsTable() + " WHERE " + this.getEnMap().GetAttrByKey(attr).getField() + " =" + this.getHisDBVarStr() + attr, ps);
	}
	public final int Delete(String attr1, Object val1, String attr2, Object val2) throws Exception
	{
		...
		return this.getHisDBSrc().RunSQL("DELETE FROM " + this.getEnMap().getPhysicsTable() + " WHERE " + this.getEnMap().GetAttrByKey(attr1).getField() + " =" + this.getHisDBVarStr() + attr1 + " AND " + this.getEnMap().GetAttrByKey(attr2).getField() + " =" + this.getHisDBVarStr() + attr2, ps);
	}
	public final int Delete(String attr1, Object val1, String attr2, Object val2, String attr3, Object val3) throws Exception
	{
		...
		return this.getHisDBSrc().RunSQL("DELETE FROM " + this.getEnMap().getPhysicsTable() + " WHERE " + this.getEnMap().GetAttrByKey(attr1).getField() + " =" + this.getHisDBVarStr() + attr1 + " AND " + this.getEnMap().GetAttrByKey(attr2).getField() + " =" + this.getHisDBVarStr() + attr2 + " AND " + this.getEnMap().GetAttrByKey(attr3).getField() + " =" + this.getHisDBVarStr() + attr3, ps);
	}
	public final int Delete(String attr1, Object val1, String attr2, Object val2, String attr3, Object val3, String attr4, Object val4) throws Exception
	{
		...
		return this.getHisDBSrc().RunSQL("DELETE FROM " + this.getEnMap().getPhysicsTable() + " WHERE " + this.getEnMap().GetAttrByKey(attr1).getField() + " =" + this.getHisDBVarStr() + attr1 + " AND " + this.getEnMap().GetAttrByKey(attr2).getField() + " =" + this.getHisDBVarStr() + attr2 + " AND " + this.getEnMap().GetAttrByKey(attr3).getField() + " =" + this.getHisDBVarStr() + attr3 + " AND " + this.getEnMap().GetAttrByKey(attr4).getField() + " =" + this.getHisDBVarStr() + attr4, ps);
	}
	...
	public final AtPara getatPara()  {
		...
		try
		{
			...
			{
				if(atParaStr.equals("0") || atParaStr.equals("undefined")) atParaStr ="";
	...
	*/
	...
	protected  int InsertExt() throws Exception {
	...
	*/
	...
	public final int Update(String key1, Object val1, String key2, Object val2) throws Exception {
		...
		key1 = key1 + "," + key2;
		return this.Update(key1.split("[,]", -1), 0);
	}
	...
	private static final int MAX_RECURSION_DEPTH = 2; // 限制重试次数，防止递归

	/**
	 更新实体
	 * @throws Exception
	*/
	public final int Update(String[] keys, int depth) throws Exception
	{
		String str = "";
		try
		{
			...
			Row row = (Row)this.getRow().clone() ;
			for(String key : row.keySet()){
				if(DataType.IsNullOrEmpty(key)==false && key.startsWith("AP_")){
					Object obj = this.getRow().get(key);
					if(obj!=null)
						this.SetPara(key, obj.toString());
				}
			}
			str = "@更新时出现错误";
		...
		catch (RuntimeException ex)
		{
					...
					|| ex.getMessage().contains("Parameter not found"))
			{
				...
				if(ex.getMessage().contains("Parameter not found")==true){
					...
					return this.Update(null, depth + 1);
				}
				//执行字段扩充检查.
				boolean isCheck = CheckPhysicsTableAutoExtFieldLength(ex);
				if (isCheck)
	...
	*/
	public final void SaveFileToDB(String saveToField, byte[] bytesOfFile) throws Exception
	{
		...
		catch (RuntimeException ex)
		{
			...
			if (DBAccess.getAppCenterDBType() == DBType.MySQL || DBAccess.getAppCenterDBType() == DBType.GBASE8CByMySQL || DBAccess.getAppCenterDBType() == DBType.GBASE8A)
			{
				sql = "ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + saveToField + " Image NULL ";
			}
	...
	*/
	...
	private DBType _DBType = DBAccess.getAppCenterDBType();
	public void setEnDBType(DBType value){
		_DBType = value;
	}
	public DBType getEnDBType() {
		String dbSrcNo = this.getEnMap().getDBSrc();
		if(DataType.IsNullOrEmpty(dbSrcNo) || dbSrcNo.equals("local"))
			return SystemConfig.getAppCenterDBType();
		return this.getEnMap().getHisDBType();
	}
	private SFDBSrc  _SFDBSrc = null;
	public SFDBSrc getHisDBSrc() throws Exception {
		String dbSrcNo = this.getEnMap().getDBSrc();
		if(DataType.IsNullOrEmpty(dbSrcNo))
			dbSrcNo = "local";
		SFDBSrc sfdbSrc = new SFDBSrc();
		if(dbSrcNo.equals("local")){
			if(_SFDBSrc == null){
				sfdbSrc.setNo(dbSrcNo);
				sfdbSrc.setDBSrcType("local");
				_SFDBSrc = sfdbSrc;
			}
			this.setEnDBType(DBAccess.getAppCenterDBType());
			return _SFDBSrc;
		}
		sfdbSrc.setNo(dbSrcNo);
		if(sfdbSrc.RetrieveFromDBSources() == 0)
			throw new RuntimeException("err@数据源["+dbSrcNo+"]没有获取到数据");
		this.setEnDBType(sfdbSrc.getHisDBType());
		return sfdbSrc;
	}
	/**
	 创建物理表
	...
	*/
	 ...
	 * @return 是否存在索引？
	 */
	public  Boolean IsExitsTableIndex(String table, String indexName) throws Exception {
		String sql = "";
		SFDBSrc sfdbSrc = this.getHisDBSrc();
		int i = 0;
		switch (this.getEnDBType())
		{
			case MSSQL:
				i = sfdbSrc.RunSQLReturnValInt("SELECT COUNT(*) FROM sys.indexes  WHERE object_id=OBJECT_ID('" + table + "', N'U') and NAME='" + indexName + "'", 0);
				break;
			case MySQL:
			case GBASE8CByMySQL:
			case GBASE8A:
				sql = "SELECT count(*) FROM information_schema.statistics WHERE table_schema='" + bp.difference.SystemConfig.getAppCenterDBDatabase() + "' AND table_name = '" + table + "' AND index_name = '" + indexName + "' ";
				i = sfdbSrc.RunSQLReturnValInt(sql);
				break;
			case PostgreSQL:
			case UX:
			case HGDB:
				sql = "SELECT count(*) FROM pg_indexes WHERE  tablename = '" + table.toLowerCase() + "' and indexname = '" + indexName.toLowerCase() + "'";
			...
			case DM:
				if (table.indexOf(".") != -1)
					table = table.split(".")[1];
				i = sfdbSrc.RunSQLReturnValInt("SELECT COUNT(*) from user_indexes   WHERE table_name= upper('" + table + "') ");
				break;
			case Oracle:
			case KingBaseR3:
			case KingBaseR6:
	...
	private void CreateIndexAndPK() throws Exception
	{
		SFDBSrc sfdbSrc = this.getHisDBSrc();
		///建立主键
		if (sfdbSrc.IsExitsTabPK(this.getEnMap().getPhysicsTable()) == false)
		{
			int pkconut = this.getPKCount();
			if (pkconut == 1)
			{
				sfdbSrc.CreatePK(this.getEnMap().getPhysicsTable(), this.getPKField(), this.getEnMap().getEnDBUrl().getDBType());
				sfdbSrc.CreatIndex(this.getEnMap().getPhysicsTable(), this.getPKField(),this.getEnMap().getEnDBUrl().getDBType());
			}
			else if (pkconut == 2)
			{
				...
				sfdbSrc.CreatePK(this.getEnMap().getPhysicsTable(), pk0, pk1, this.getEnMap().getEnDBUrl().getDBType());
				sfdbSrc.CreatIndex(this.getEnMap().getPhysicsTable(), pk0, pk1);
			}
			else if (pkconut == 3)
			{
				String pk0 = this.getPKs()[0];
				...
				sfdbSrc.CreatePK(this.getEnMap().getPhysicsTable(), pk0, pk1, pk2, this.getEnMap().getEnDBUrl().getDBType());
				sfdbSrc.CreatIndex(this.getEnMap().getPhysicsTable(), pk0, pk1, pk2);
			}
		}
	}
	/**
	...
	*/
	...
	private void CheckPhysicsTable_SQL(SFDBSrc sfdbSrc) throws Exception
	{
		String table = this.getEnMap().getPhysicsTable();
		DBType dbtype = this.getEnDBType();
		String sqlFields = "";
		String sqlYueShu = "";


		DataTable dtAttr = sfdbSrc.RunSQLReturnTable(DBAccess.SQLOfTableFieldDesc(table,dbtype));
		DataTable dtYueShu = sfdbSrc.RunSQLReturnTable(DBAccess.SQLOfTableFieldYueShu(table,dbtype));
				...
				switch (attr.getMyDataType())
				{
					...
					case DataType.AppDateTime:
						...
						if (len > 4000)
						{
							...
							{
								sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + "  Memo DEFAULT '" + attr.getDefaultVal() + "' NULL");
							}
							else
							{
								sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " NVARCHAR(MAX) DEFAULT '" + attr.getDefaultVal() + "' NULL");
							}
						}
						else
						{
							...
							{
								sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " NVARCHAR(" + len + ") DEFAULT '" + attr.getDefaultVal() + "' NULL");
							}
						}
						continue;
					case DataType.AppInt:
						if(attr.getDefaultVal() == null)
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT NULL");
						else
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT '" + attr.getDefaultVal() + "' NULL");
						continue;
					case DataType.AppBoolean:
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT '" + attr.getDefaultVal() + "' NULL");
						continue;
					...
					case DataType.AppDouble:
						if(attr.getDefaultVal() == null)
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " FLOAT DEFAULT  NULL");
						else
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " FLOAT DEFAULT '" + attr.getDefaultVal() + "' NULL");
						continue;
					default:
				...
				case DataType.AppDateTime:
					if (FType.toLowerCase().contains("char"))
					{
						...
						if (len < attr.getMaxLength())
						{
							try
							{
								sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ALTER column " + attr.getKey() + " VARCHAR(" + attr.getMaxLength() + ")");
							}
							catch (java.lang.Exception e)
							{
								...
								for (DataRow dr : dtYueShu.Rows)
								{
									...
									{
										sfdbSrc.RunSQL("ALTER TABLE " + table + " drop constraint " + dr.getValue(0).toString());
									}
								}
								...
								{
									sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ALTER column " + attr.getKey() + " VARCHAR(4000)");
								}
								else
								{
									sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ALTER column " + attr.getKey() + " VARCHAR(" + attr.getMaxLength() + ")");
		...
		for (Attr item : this.getEnMap().getAttrs())
		{
			...
			for (DataRow dr : dtAttr.Rows)
			{
				String fName = dr.getValue("FName").toString();
				if (fName.equals(item.getKey()) == true)
				{
					isHave = true;
					break;
				}
	 ...
	 * @throws Exception
	*/
	private void CheckPhysicsTable_PostgreSQL(SFDBSrc sfdbSrc) throws Exception
	{
		String table = this.getEnMap().getPhysicsTable();
		...
		DataTable dtAttr = sfdbSrc.RunSQLReturnTable(sqlFields);
				...
				switch (attr.getMyDataType())
				{
					...
					case DataType.AppDateTime:
						...
						if (len > 4000)
						{
							...
							{
								sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " VARCHAR(" + len + ") DEFAULT '" + attr.getDefaultVal() + "' NULL");
							}
						}
						else
						{
						...
						sfdbSrc.RunSQL("COMMENT ON COLUMN " + this.getEnMap().getPhysicsTable() + "." + attr.getField() + " IS '" + attr.getDesc() + "';");
						continue;
					case DataType.AppInt:
						if(attr.getDefaultVal() == null)
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT  NULL");
				...
				case DataType.AppDateTime:
					if (FType.toLowerCase().contains("char"))
					{
						...
						if (len < attr.getMaxLength())
						{
							try
							{
								sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ALTER column " + attr.getKey() + " type character varying(" + attr.getMaxLength() + ")");
							}
							catch (java.lang.Exception e)
							{
								/*如果类型不匹配，就删除它在重新建, 先删除约束，在删除列，在重建。*/
					...
					else
					{
						...
						/***如果类型不匹配，就删除它在重新建, 先删除约束，在删除列，在重建。*/
						String atlerSql="ALTER TABLE "+this.getEnMap().getPhysicsTable()+" ALTER COLUMN "+attr.getField()+"  TYPE varchar("+attr.getMaxLength()+");";
						String alterSqlDefault="ALTER TABLE "+this.getEnMap().getPhysicsTable()+" ALTER COLUMN "+attr.getField()+" set DEFAULT '" + attr.getDefaultVal() + "'";
						sfdbSrc.RunSQL(atlerSql+alterSqlDefault);
						continue;
					}
					break;
				case DataType.AppInt:
		...
		{
			sfdbSrc.CreatIndex(this.getEnMap().getPhysicsTable(), this.getEnMap().IndexField, this.getEnMap().getEnDBUrl().getDBType());
		}
	...
	*/
	public final boolean CheckPhysicsTableAutoExtFieldLength(RuntimeException ex) throws Exception
	{
		...
		switch (this.get_enMap().getHisDBType())
		{
			case MSSQL:
	...
	//记录当前检查的表，如果是同一个实体就结束
	private static String checkEntityName = "";
	/**
	 检查物理表
	 * @throws Exception
	*/
	...
	{
		Map enMap  = this.getEnMap();
		this.set_enMap(enMap);

		SFDBSrc sfdbSrc = this.getHisDBSrc();
		if (enMap.getEnType() == EnType.View || enMap.getEnType() == EnType.XML || enMap.getEnType() == EnType.ThirdPartApp || enMap.getEnType() == EnType.Ext)
		{
			return;
		}

		if (sfdbSrc.IsExitsObject(enMap.getPhysicsTable()) == false)
		{
			/* 如果物理表不存在就新建立一个物理表。*/
		...
		if(sfdbSrc.getDBSrcType().equals("local"))
			isView = enMap.getIsView();
		else
			isView = sfdbSrc.IsView(enMap.getPhysicsTableExt());
		if (isView)
		{
			return;
		}
		// 如果不是主应用程序的数据库就不让执行检查. 考虑第三方的系统的安全问题.
		if (enMap.getEnDBUrl().getDBUrlType() != DBUrlType.AppCenterDSN)
		{
			return;
		}
		//检查是否有对应的主键.
		String pk = this.getPK();
		if (pk.contains(",") == false && enMap.getAttrs().contains(pk) == false)
		{
			if (this.toString().contains(".") == true)
			{
				throw new RuntimeException("err@Entity " + this.toString() + "," + enMap.getEnDesc() + "的Map设置错误主键为【" + pk + "】但是没有这个字段，请检查Map。");
			}
			else
			{
				if (pk.equals("OID") == true)
				{
					MapAttr attr = new MapAttr();
					attr.setFrmID(this.toString());
					attr.setKeyOfEn("OID");
				...
				{
					throw new RuntimeException("err@Entity " + this.toString() + "," + enMap.getEnDesc() + "的Map设置错误主键为【" + pk + "】但是没有这个字段，请检查:  SELECT * FROM Sys_MapAttr WHERE FK_MapData='" + this.toString() + "'");
				}
			}
		}

		//说明同时调用了检查表的字段,如果一个检查
		if (checkEntityName.equals(this.getClassID()) == true)
		{
			while (checkEntityName.equals("")==false)
			{
				Thread.sleep(10);
			}
			return;
		}
		checkEntityName = this.getClassID();
		try{
			switch (this.getEnDBType())
			{
				case MSSQL:
					this.CheckPhysicsTable_SQL(sfdbSrc);
					break;
				case Oracle:
				case DM:
					this.CheckPhysicsTable_Ora(sfdbSrc);
					break;
				case MySQL:
				case GBASE8CByMySQL:
				case GBASE8A:
					this.CheckPhysicsTable_MySQL(sfdbSrc);
					break;
				case Informix:
				...
				case HGDB:
				case GBASE8CByOracle:
					this.CheckPhysicsTable_PostgreSQL(sfdbSrc);
					break;
				case KingBaseR3:
				case KingBaseR6:
					this.CheckPhysicsTable_King(sfdbSrc);
					break;
				default:
					throw new RuntimeException("@没有涉及到的数据库类型");
			}
			...
			checkEntityName = "";
			throw new RuntimeException("err@系统错误,ccbpm有智能修复功能,请关闭后重试,也许可以自动解决,没有解决请反馈给管理员:" + ex.getMessage());
		}
	...
	private void CheckPhysicsTable_MySQL(SFDBSrc sfdbSrc) throws Exception
	{

			///检查字段是否存在
		String sql = "SELECT *  FROM " + this.getEnMap().getPhysicsTable() + " WHERE 1=2";
		DataTable dt = sfdbSrc.RunSQLReturnTable(sql);

		//如果不存在.
		Attrs attrs = this.getEnMap().getAttrs();
		for (Attr attr : attrs)
		{
			if (DataType.IsNullOrEmpty(attr.getField()) == true)
				continue;
			if (attr.getMyFieldType() == FieldType.RefText)
			{
				continue;
			...
			{
				/* 自动增长列 */
				sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT  Identity(1,1) COMMENT '" + attr.getDesc() + "'");
				continue;
			}
			...
			switch (attr.getMyDataType())
			{
				case DataType.AppString:
					...
					if (len > 3000)
					{
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " TEXT COMMENT '" + attr.getDesc() + "'");
					}
					else
					{
						if(SystemConfig.getAppCenterDBType() == DBType.GBASE8A && (attr.getField().equals("Sort") ||attr.getField().equals("Target")))
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD \"" + attr.getField() + "\" NVARCHAR(" + len + ") DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
						else
							sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " NVARCHAR(" + len + ") DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					}
					break;
				case DataType.AppDate:
				case DataType.AppDateTime:
					sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " NVARCHAR(20) DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					break;
				case DataType.AppInt:
				case DataType.AppBoolean:
					if( attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					break;
				case DataType.AppFloat:
					if( attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " FLOAT (11,2) DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " FLOAT (11,2) DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					break;
				case DataType.AppMoney:
					if( attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " DECIMAL (20,"+attr.getDefaultDecimalLen()+") DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " DECIMAL (20,"+attr.getDefaultDecimalLen()+") DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					break;
				case DataType.AppDouble:
					if( attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " DOUBLE DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " DOUBLE DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					break;
				default:
					throw new RuntimeException("error MyFieldType= " + attr.getMyFieldType() + " key=" + attr.getKey());
			}
		...
		for (Attr attr : this.getEnMap().getAttrs()){
			if (attr.getMyFieldType() == FieldType.RefText)
				continue;
			DataColumn dc = dt.Columns.get(attr.getField());
			if(dc==null)
				continue;
			if(SystemConfig.getAppCenterDBType() == DBType.GBASE8A){
				if(attr.getMyDataType()==DataType.AppString && dc.DataType == String.class)
					continue;
				if((attr.getMyDataType()==DataType.AppInt || attr.getMyDataType()==DataType.AppBoolean) && dc.DataType == Integer.class)
					continue;
				if((attr.getMyDataType()==DataType.AppFloat || attr.getMyDataType()==DataType.AppDouble || attr.getMyDataType()==DataType.AppMoney)&& dc.DataType == Float.class)
					continue;
				//不能直接改类型，需要删除后重新创建
				//暂时不能修改类型
				continue;
			...
			//数字类型转字符类型
			if(attr.getMyDataType()==DataType.AppString) {
				if (dc.DataType == Integer.class || dc.DataType == Float.class || dc.DataType == Double.class) {
					//修改字段类型
					int len = attr.getMaxLength();
					if (len == 0) {
						len = 200;
					...
					if (len > 3000) {
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " TEXT COMMENT '" + attr.getDesc() + "'");
					} else {
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " NVARCHAR(" + len + ") DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
					}
					break;
				}
				continue;
			...
			if(attr.getMyDataType() == DataType.AppInt || attr.getMyDataType() == DataType.AppBoolean){
				if(dc.DataType != Integer.class){
					if(attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " INT DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " INT DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
				}
				continue;
			}
			if (attr.getMyDataType() == DataType.AppFloat) {
				if(attr.getDefaultVal() == null)
					sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " FLOAT (11,2) DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
				else
					sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " FLOAT (11,2) DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
				continue;
			}
			if(attr.getMyDataType() == DataType.AppDouble ){
				if(dc.DataType != Float.class){
					if(attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " DOUBLE DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " DOUBLE DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
				}
				continue;
			}
			if(attr.getMyDataType() == DataType.AppMoney ){
				if(dc.DataType != Float.class){
					if(attr.getDefaultVal() == null)
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " DECIMAL (20,"+attr.getDefaultDecimalLen()+") DEFAULT  NULL COMMENT '" + attr.getDesc() + "'");
					else
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " MODIFY " + attr.getField() + " DECIMAL (20,"+attr.getDefaultDecimalLen()+") DEFAULT '" + attr.getDefaultVal() + "' NULL COMMENT '" + attr.getDesc() + "'");
				}
				continue;
			}
		}
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			...
			for (DataRow dr : dt.Rows)
			{
				try
				{
					...
					{
						sfdbSrc.RunSQL("alter table " + dr.getValue("OWNER") + "." + this.getEnMap().getPhysicsTableExt() + " modify " + attr.getField() + " NVARCHAR(" + attr.getMaxLength() + ")");
					}
					else
					{
						sfdbSrc.RunSQL("alter table " + dr.getValue("OWNER") + "." + this.getEnMap().getPhysicsTableExt() + " modify " + attr.getField() + " text");
					}
				}
				catch (RuntimeException ex)
				{
			...
			///检查枚举类型字段是否是INT 类型
		Attrs enumAttrs = this.getEnMap().getHisEnumAttrs();
		for (Attr attr : enumAttrs)
		{
			...
			if (val.indexOf("CHAR") != -1)
			{
				...
				try
				{
					sfdbSrc.RunSQL("alter table  " + this.getEnMap().getPhysicsTableExt() + " modify " + attr.getField() + " NUMBER ");
				}
				catch (RuntimeException ex)
				{
	...
	private void CheckPhysicsTable_Ora(SFDBSrc sfdbSrc) throws Exception
	{
		DataTable dt = new DataTable();
		String sql = "SELECT LISTAGG(DISTINCT(column_name),',') AS Column_Name  FROM all_tab_cols WHERE table_name = '" + this.getEnMap().getPhysicsTable().toUpperCase() + "' AND owner='" +  SystemConfig.getUser().toUpperCase() + "'";
		try{
			if(this.getEnDBType() != DBType.Oracle){
				sql = "SELECT WMSYS.WM_CONCAT(DISTINCT(column_name)) AS Column_Name  FROM all_tab_cols WHERE table_name = '" + this.getEnMap().getPhysicsTable().toUpperCase() + "' AND owner='" +  SystemConfig.getUser().toUpperCase() + "'";
			}
			dt = sfdbSrc.RunSQLReturnTable(sql);
		}catch(Exception e){
			Log.DebugWriteError("err@ORACLE执行SQL出错，SQL=[" + sql + "]");
		}
		if (dt.Rows.size() == 0)
		{
			return;
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			...
			{
				/* 自动增长列 */
				sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT  Identity(1,1)");
				continue;
			}
			...
			switch (attr.getMyDataType())
			{
				...
				case DataType.AppBoolean:
					...
					{
						sfdbSrc.RunSQL("ALTER TABLE " + this.getEnMap().getPhysicsTable() + " ADD " + attr.getField() + " INT DEFAULT '" + attr.getDefaultVal() + "' NOT NULL");
					}
					else
					{
						if(attr.getDefaultVal() == null)
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			...
			for (DataRow dr : dt.Rows)
			{
				...
				sfdbSrc.RunSQL("alter table " + this.getEnMap().getPhysicsTableExt() + " modify " + attr.getField() + " varchar2(" + attr.getMaxLength() + ")");
		...
		for (Attr attr : attrs)
		{
			...
			String val = sfdbSrc.RunSQLReturnString(sql);
			if (val == null)
			{
				Log.DefaultLogWriteLineError("@没有检测到字段eunm" + attr.getKey());
			}
			if (val.indexOf("CHAR") != -1)
			{
				...
				String OWNER = sfdbSrc.RunSQLReturnString(sql);
				try
				{
					sfdbSrc.RunSQL("alter table  " + this.getEnMap().getPhysicsTableExt() + " modify " + attr.getField() + " NUMBER ");
				}
	 ...
	 * 人大金仓检查表字段
	 */
	private void CheckPhysicsTable_King(SFDBSrc sfdbSrc) throws Exception
	{
		...
		for (Attr attr : this.getEnMap().getAttrs())
		{
			...
			for (DataRow dr : dt.Rows)
			{
				sfdbSrc.RunSQL("alter table " + this.getEnMap().getPhysicsTableExt() + "  ALTER COLUMN " + attr.getField() + " TYPE varchar2(" + attr.getMaxLength() + ")");
			}
		}
		...
		for (Attr attr : attrs)
		{
			...
			if (val.indexOf("CHAR") != -1)
			{
				...
				try
				{
					sfdbSrc.RunSQL("alter table  " + this.getEnMap().getPhysicsTableExt() + " ALTER COLUMN " + attr.getField() + " TYPE NUMBER ");
				}
				catch (RuntimeException ex)
				{
	...
	*/
	 ...
	 * @throws Exception
	*/
	private void DTSMapToSys_MapData_InitMapAttr(Attrs attrs, String frmID) throws Exception
	{
		for (Attr attr : attrs)
		{
			if (attr.getItIsRefAttr() )
