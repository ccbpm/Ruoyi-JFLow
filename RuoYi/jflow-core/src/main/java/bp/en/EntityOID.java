	...
	*/
	public final void InsertAsOID(int oid) throws Exception {
		...
		try
		{
			...
			switch (sfdbSrc.getHisDBType())
			{
				case MSSQL:
					sfdbSrc.RunSQL(this.getSQLCache().Insert, SqlBuilder.GenerParas(this, null));
				case Access:
					sfdbSrc.RunSQL(this.getSQLCache().Insert, SqlBuilder.GenerParas(this, null));
				case MySQL:
				case Informix:
				default:
					sfdbSrc.RunSQL(this.getSQLCache().Insert.replace("[", "").replace("]", ""), SqlBuilder.GenerParas(this, null));
			}
			//this.RunSQL(SqlBuilder.Insert(this));
		}
		catch (RuntimeException ex)
