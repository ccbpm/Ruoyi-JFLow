...
public class TemplateNode extends Entity
{
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBString("AiLab", "Ai助理", "Ai助理", true, false, 0, 50, 10);
		map.AddBoolean("AiAssistant", false, "是否启用", true, true);
		if (SystemConfig.getCustomerNo() == "TianYu") {
			map.AddTBString("CanCelLab", "作废", "作废按钮标签", true, false, 0, 50, 10);
			map.AddBoolean("CanCelEnable", false, "是否启用", true, true);
		}
		map.AddTBString("TZWorkerLab", "调整未来处理人", "调整未来处理人", true, false, 0, 50, 10);
		map.AddBoolean("TZWorkerEnable", false, "是否启用调整未来处理人?", true, true);

		map.AddTBInt("TZWorkerRole", 0, "调整规则",false,true);
		...
		map.AddTBInt("GovDocSta", 0, "公文组件", false, true);
		map.AddBoolean("WaitIframeMsg", false, "是否等待嵌入式表单消息", true, true, true);
