...
public class WF_Admin_CCBPMDesigner extends bp.difference.handler.DirectoryPageBase
{
	...
	*/
	 ...
	 * @return
	 */
	public final String FlowCopy() throws Exception {
		...
		//执行导入
		Flow newFlow = TemplateGlo.LoadFlowTemplate(flowSort, fileXml, ImpFlowTempleteModel.AsNewFlow, flowNo);
		newFlow.setPTable("ND"+Integer.parseInt(newFlow.getNo())+"Rpt");
		newFlow.SetValByKey(FlowAttr.CreateDate,DataType.getCurrentDateTime());
		newFlow.setVer(DataType.getCurrentDateTime());
		newFlow.setName(flow.getName() + "Copy");
		if (SystemConfig.getCCBPMRunModel() == CCBPMRunModel.SAAS){
			newFlow.setOrgNo(flowSort);
		}
		newFlow.Update();
		newFlow.DoCheck(); //要执行一次检查.、
	...
	*/
	public final String Default_Init()
	{
		try
		{
			...
			{
				// 执行升级
				String str = Upgrader.UpdataCCFlowVer();
				if (str == null)
				{
					str = "";
				}
	...
	*/
	public final String Login_Init() throws Exception {
		...
		try
		{
			...
			if (str == null)
			{
				str = "准备完毕,欢迎登录,当前小版本号为:" + Upgrader.Ver;
			}
			return str;
		}
		catch (RuntimeException ex)
