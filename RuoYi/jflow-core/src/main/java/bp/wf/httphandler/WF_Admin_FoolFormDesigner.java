...
public class WF_Admin_FoolFormDesigner extends bp.difference.handler.DirectoryPageBase
{
	...
	*/
	public final String Designer_CheckFrm() throws Exception {
		...
		boolean isDtlMapData = this.GetRequestValBoolen("IsDtl");
		if(i == 0){
			mymd.setName(this.GetRequestVal("FrmName"));
			mymd.setFormEventEntity("");
			mymd.setEnPK("");
			mymd.Insert();
		}
		mymd.RepairMap(isDtlMapData);
		//清缓存
		mymd.ClearCache();
			///#endregion
	...
	*/
	public final String Designer_Init() throws Exception {
        ...
        paras.SQL = "DELETE FROM Sys_GroupField " +
                "WHERE NOT EXISTS ( " +
                "    SELECT 1 FROM Sys_MapAttr " +
                "    WHERE Sys_MapAttr.groupID = Sys_GroupField.oid " +
                "    AND Sys_MapAttr.FK_MapData = " + dbStr + "FK_MapData" +
                ") " +
                "AND ctrlType = '' " +
                "AND lab = '' " +
                "AND FrmID = " + dbStr + "FrmID";
        paras.Add("FK_MapData",frmID);
		paras.Add("FrmID",frmID);
		DBAccess.RunSQL(paras);
        //如果是第一次进入，就执行旧版本的升级检查.
        if (this.getItIsFirst()) {
        ...
        // 使用CompletableFuture来并行加载所有数据
        CompletableFuture<Void> futureMapData = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                MapData md = new MapData();
                md.setNo(frmID);
                md.RetrieveFromDBSources();
                return md.ToDataTableField("Sys_MapData");
            } catch (Exception e) {
                throw new CompletionException(e);
            }
        }, threadPoolTaskExecutor).thenAccept(table -> {
            synchronized (ds) {
                ds.Tables.add(table);
            }
        });

        CompletableFuture<Void> futureMapAttr = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                // 字段属性.
                MapAttrs mattrs = new MapAttrs(frmID);
                for (MapAttr item : mattrs.ToJavaList()) {
                    item.setDefVal(item.getDefValReal());
                }
                return mattrs.ToDataTableField("Sys_MapAttr");
            } catch (Exception e) {
                throw new CompletionException(e);
            }
        }, threadPoolTaskExecutor).thenAccept(table -> {
        ...
        CompletableFuture<Void> futureDtl = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                MapDtls dtls = new MapDtls();
                dtls.Retrieve(MapDtlAttr.FK_MapData, frmID, MapDtlAttr.FK_Node, 0, null);
                ...
                return new DataTable[]{
                        dtls.ToDataTableField("Sys_MapDtl"),
                        gfs.ToDataTableField("Sys_GroupField")
                };
            } catch (Exception e) {
                throw new CompletionException(e);
            }
        }, threadPoolTaskExecutor).thenAccept(tables -> {
            synchronized (ds) {
        ...
        CompletableFuture<Void> futureAth = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                //附件表.
                FrmAttachments aths = new FrmAttachments(frmID);
                return aths.ToDataTableField("Sys_FrmAttachment");
            } catch (Exception e) {
                throw new CompletionException(e);
            }
        }, threadPoolTaskExecutor).thenAccept(table -> {
        ...
        CompletableFuture<Void> futureExt = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                //加入扩展属性.
                MapExts MapExts = new MapExts(frmID);
        ...
        CompletableFuture<Void> futureFrame = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                //加入扩展属性.
                MapFrames frms = new MapFrames(frmID);
                return frms.ToDataTableField("Sys_MapFrame");
            } catch (Exception e) {
                throw new CompletionException(e);
            }
        }, threadPoolTaskExecutor).thenAccept(table -> {
        ...
        CompletableFuture<Void> futureNode = CompletableFuture.supplyAsync(() -> {
            RequestContextHolder.setRequestAttributes(requestAttributes);
            try {
                // 处理可能的工作流节点情况
                if (frmID.startsWith("ND")) {
                    String nodeStr = this.getFrmID().replace("ND", "");
                    if (DataType.IsNumStr(nodeStr)) {
                        FrmNodeComponent fnc = new FrmNodeComponent(Integer.parseInt(nodeStr));
                        return fnc.ToDataTableField("WF_Node");
                    }
                }
                return null;
            } catch (Exception e) {
        ...
        // 创建一个组合所有CompletableFuture的单一future
        CompletableFuture<Void> allFutures = CompletableFuture.allOf(
                futureMapData, futureMapAttr, futureDtl, futureAth, futureExt, futureFrame, futureNode
        );

        // 等待所有任务完成
        try {
            allFutures.get(10, TimeUnit.SECONDS); // 等待所有操作完成，有异常会抛出
        } catch (Exception e) {
            // 处理异常
            throw new RuntimeException("加载表单数据时发生错误", e);
        }
	...
	*/
	public final String SFGuide_GetClass() throws Exception
	{
		...
		Entities ens = null;
		SFTable sf = null;
		sfs.Retrieve(SFTableAttr.DBSrcType, DictSrcType.BPClass);
