...
public class WF_Admin_AttrNode_FrmSln extends bp.difference.handler.DirectoryPageBase
{
	...
	*/
	...
	/**
	 嵌入式表单-设置该流程的所有节点都是用该方案。

	 @return
	 */
	public final String SelfForm_SetAllNodeFrmUseThisSln() throws Exception {
		...
		for (int i = 0; i < nds.size(); i++)
		{
			...
			BtnLab jsonBtn  = new BtnLab(jsNode.getNodeID());

			//修改按钮 保存，轨迹，回滚
			DBAccess.RunSQL("UPDATE WF_Node SET SaveEnable="+(btn.getSaveEnable()?1:0)+" WHERE NodeID ='"+jsNode.getNodeID()+"'");
			DBAccess.RunSQL("UPDATE WF_Node SET TrackEnable="+(btn.getTrackEnable()?1:0)+" WHERE NodeID ='"+jsNode.getNodeID()+"'");
			DBAccess.RunSQL("UPDATE WF_Node SET RollbackEnable="+(btn.getRollbackEnable()?1:0)+" WHERE NodeID ='"+jsNode.getNodeID()+"'");

			//修改表单
			jsNode.setFormType(currNode.getFormType());
			jsNode.setFormUrl( currNode.getFormUrl());
			jsNode.SetValByKey(NodeAttr.FormUrlMobile, currNode.GetValStringByKey(NodeAttr.FormUrlMobile));
			jsNode.SetPara("SelfFormEnRoot", currNode.GetParaInt("SelfFormEnRoot"));
			jsNode.Update();
		}

		Flow flow = new Flow(currNode.getFlowNo());
		flow.setFrmUrl(currNode.getFormUrl());
		flow.setFlowDevModel(FlowDevModel.SelfFrm);
		flow.SetPara(NodeAttr.FormUrlMobile, currNode.GetValStringByKey(NodeAttr.FormUrlMobile));
		flow.SetPara(NodeAttr.SelfFormEnRoot, currNode.GetParaInt(NodeAttr.SelfFormEnRoot));
		flow.DirectUpdate();
	...
	*/
	public final String RefOneFrmTree_SetAllNodeFrmUseThisSln() throws Exception {
		...
		for (int i = 0; i < nds.size(); i++)
		{
			...
			jsNode.SetValByKey(NodeAttr.NodeFrmIDT,currNode.GetValStringByKey(NodeAttr.NodeFrmIDT));
			jsNode.Update();
			...
			for (int idx = 0; idx < ens.size(); idx++)
			{
				...
				if (!Objects.equals(en.getFrmID(), currNode.getNodeFrmID()))
				{
			...
			frmNode.setFrmID(jsNode.getNodeFrmID());
		...
		//更新模式.
		Flow flow = new Flow(currNode.getFlowNo());
		flow.setFrmUrl(currNode.getNodeFrmID());
		flow.setFlowDevModel(FlowDevModel.RefOneFrmTree);
		flow.Update();

		return "执行成功,建议您关闭浏览器重新打卡.";
	}
	/**
	 设置该流程的所有节点都是用该方案。
	 ...
	 @return
	 */
	public final String SheetTree_SetAllNodeFrmUseThisSln() throws Exception {
		//1.获取当前的节点信息
		Node currNode = new Node(this.getNodeID());
		...
		//3.获取当前节点绑定的表单
		FrmNodes frmNodes = new FrmNodes(flowNo,currNode.getNodeID());

		//4.获取自定义的字段
		FrmFields frmFields = new FrmFields();
		frmFields.Retrieve(FrmFieldAttr.FK_Node,currNode.getNodeID());

		//获取绑定表单的组件
		FrmNode en = null;
		FrmField frmField = null;
		for(Node nd :nds.ToJavaList()){
			if(nd.getNodeID() == currNode.getNodeID())
				continue;
			nd.setHisFormType(NodeFormType.SheetTree);
			nd.Update();
			//删除当前节点绑定的表单
			FrmNodes ens = new FrmNodes();
			ens.Delete(FrmNodeAttr.FK_Node, nd.getNodeID());
			for(FrmNode frmNode : frmNodes.ToJavaList()){
				en = new FrmNode();
				en.Copy(frmNode);
				en.setNodeID(nd.getNodeID());
				en.setMyPK(frmNode.getFrmID() + "_" + nd.getNodeID() + "_" + currNode.getFlowNo());
				if(nd.getItIsStartNode())
					en.setFrmSln(FrmSln.Default); //默认方案
				en.DirectInsert();
			}
			//删除表单权限
			FrmFields fields = new FrmFields();
			frmFields.Delete(FrmFieldAttr.FK_Node,nd.getNodeID());
			for(FrmField field : fields.ToJavaList()){
				frmField = new FrmField();
				frmField.Copy(field);
				frmField.setNodeID(nd.getNodeID());
				frmField.setMyPK(frmField.getFrmID() + "_" + nd.getNodeID() + "_" + frmField.getKeyOfEn());
				if (frmField.IsExits() == false)
				{
