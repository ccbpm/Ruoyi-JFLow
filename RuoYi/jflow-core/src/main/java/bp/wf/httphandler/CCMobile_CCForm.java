...
public class CCMobile_CCForm extends bp.difference.handler.DirectoryPageBase
{
	...
	*/
	public final String getAccessToken()  {
		String ak = SystemConfig.getBaiDuAPIKey();
		String sk = SystemConfig.getBaiDuSecretKey();
	...
	*/
	public final String GetWXGZHConfigSetting() throws Exception {
		...
		String jsapi_ticket = "";
        String url1 = this.GetRequestVal("htmlPage");
		//获取 AccessToken
		String accessToken = bp.wf.difference.Glo.GetWXGZHToken();
		JSONObject tokenJson = JSONObject.fromObject(accessToken);

		//判断是否存在异常
		if(tokenJson.containsKey("errcode")){
			return "err@获取网页授权失败，微信服务端响应：" + accessToken;
		}

		String token = tokenJson.getString("access_token");

		String ticketStr = bp.wf.difference.Glo.GetWXGZHJsapiTicket(token);
		JSONObject ticketJson = JSONObject.fromObject(ticketStr);

		//权限签名算法
		if(ticketJson.getInt("errcode") != 0) {
