...
public class GenerWorkFlowExt extends Entity
{
	...
	*/
	...
	public final void setWFState(WFState value) throws Exception {
		if (value == WFState.CompleteEnd || value == WFState.CompleteEnd)
		{
	...
	*/
	public final String RepairDataIt() throws Exception {
		...
		bp.wf.GERpt rpt = fl.getHisGERpt();
		rpt.setOID(this.getWorkID());
		int i= rpt.RetrieveFromDBSources();
		if (i==0)
		{
			//没有数据就插入.
			rpt.setFlowStartRDT(this.getRDT());
			rpt.setTitle(this.getTitle());
			rpt.setFlowStarter(this.getStarter());
			rpt.setNY(DataType.getCurrentYearMonth());
			rpt.setFlowEmps(this.getEmps());
			rpt.setFlowEnderRDT(this.getSDTOfNode());
			rpt.setDeptNo(this.getFK_Dept());
			rpt.setDeptName(this.getDeptName());
			rpt.setFlowEnder(this.getTodoEmps());
			rpt.setFlowEndNode(this.getFK_Node());
			rpt.setWFState(this.getWFState());
			rpt.setFID(this.getFID());
			rpt.SaveAsOID(this.getWorkID());
		}
	...
	*/
	...
	public String DoFlowOver(String note) throws Exception {
		Dev2Interface.Flow_DoFlowOver(this.getWorkID(), note);
