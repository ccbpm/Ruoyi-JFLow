...
public class FrmEvent extends EntityMyPK
{
	...
	*/
	public final int getNodeID()  {
		return this.GetValIntByKey(FrmEventAttr.NodeID);
	}
	public final void setNodeID(int value){
		this.SetValByKey(FrmEventAttr.NodeID, value);
	}
	/**
	 关联的流程编号
	*/
	...
	public final String getRefPKVal()  {
		return this.GetValStringByKey(FrmEventAttr.RefPKVal);
	}
	public final void setRefPKVal(String value){
		this.SetValByKey(FrmEventAttr.RefPKVal, value);
	}
	/**
	 流程
	*/
	public final String getFlowNo()  {
		return this.GetValStringByKey(FrmEventAttr.FlowNo);
	}
	public final void setFlowNo(String value){
		this.SetValByKey(FrmEventAttr.FlowNo, value);
	}
	/**
	 节点
	*/
	...
	*/
	...
	public Map getEnMap()
	{
		...
		map.AddTBString(FrmEventAttr.RefPKVal, null, "关联主键", true, true, 0, 10, 10);

		//事件类型的主键.
		map.AddTBString(FrmEventAttr.FrmID, null, "表单ID", true, true, 0, 100, 10);
		map.AddTBString(FrmEventAttr.FlowNo, null, "流程编号", true, true, 0, 100, 10);
		map.AddTBInt(FrmEventAttr.NodeID, 0, "节点ID", true, true);

		//执行内容. EventDoType 0=SQL,1=URL....
		map.AddTBString(FrmEventAttr.EventDoType, null, "事件执行类型", true, true,0,100,100);
		map.AddTBString("EventDoTypeT", null, "事件执行类型T", true, false, 0, 100, 100);
		...
		map.AddTBString("RequestMethod", "Get", "请求模式", true, true, 0, 100, 10);
		...
		map.AddTBString("PostDoc", null, "POST内容", true, true, 0, 500, 10);//测试用例.
		map.AddTBString("HeaderDoc", null, "Header内容", true, true, 0, 500, 10);//测试用例.


		map.AddTBString("TestParas", "0", "测试数据", true, true, 0, 500, 10);//测试用例.
	...
	public String ToTestWebApi()
	{
		String testParas = this.GetValStrByKey("TestParas");
		if (DataType.IsNullOrEmpty(testParas) == true)
			return "err@请按照要求配置测试的输入参数.";

    	//#region 1.整理参数.
			AtPara ap = new AtPara(testParas);

		String PostModel = this.GetValStrByKey("RequestMethod").toUpperCase();
		String url = this.getDoDoc();
		url = bp.difference.Glo.DealExp(url, ap.getHisHT());

		String PostDoc = this.GetValStrByKey("PostDoc");
		PostDoc = PostDoc.replace("~~", "\"");
		PostDoc = PostDoc.replace("~", "\"");
		PostDoc = bp.difference.Glo.DealExp(PostDoc, ap.getHisHT());

		String HeaderDoc = this.GetValStrByKey("HeaderDoc");
		HeaderDoc = PostDoc.replace("~~", "\"");
		HeaderDoc = PostDoc.replace("~", "\"");
		HeaderDoc = bp.difference.Glo.DealExp(HeaderDoc, ap.getHisHT());
    	//#endregion 整理参数.

		ResultObj obj= bp.en.OverrideFile.Data_WebApi_GenerStandReturnObj(PostModel,url, PostDoc, HeaderDoc);
