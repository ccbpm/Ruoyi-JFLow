...
public class CCFormAPI
{
	...
	private static final Logger logger = LoggerFactory.getLogger(CCFormAPI.class);


	///region 智能判断字段类型.
	//3-float  2-int 4-boolen
	public static String fieldNameKeys = "[{\"regName\":\"地点|场所|位置\",\"dataType\":\"1\"}," +
			"{\"regName\":\"费用|费|计|金额|额|值|价|金\",\"dataType\":\"8\"}," +
			"{\"regName\":\"日期\",\"dataType\":\"6\"}," +
			"{\"regName\":\"时间\",\"dataType\":\"7\"}," +
			"{\"regName\":\"长|宽|高|公斤|斤|率|重|公里|里程|千克\",\"dataType\":\"3\"}," +
			"{\"regName\":\"个|量|数|台|张|分数|分|只|件|把|张|条|双|瓶|盒|袋|堆|群|串|溜|阵|批|幅|截|缕|抹|丝|毫|点|滴|片|粒|颗|瓣|丛|龄|簇\",\"dataType\":\"2\"}," +
			"{\"regName\":\"否|是|是否\",\"dataType\":\"4\"}]";

	// //后置提示endLab图标let icon提示tip;
	public static String fieldEndLabs = "[{\"unitEndLabName\":\"KG\",\"regName\":\"体重|重量|重\",\"icon\":\"icon-user\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"CM\",\"regName\":\"身高|身长|尺寸|长|宽|高\",\"icon\":\"\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"公里\",\"regName\":\"距离|里程|路程\",\"icon\":\"\",\"tip\":\"请输入公里数\"}," +
			"{\"unitEndLabName\":\"周岁\",\"regName\":\"龄|年纪\",\"icon\":\"\",\"tip\":\"请输入年龄\"}," +
			"{\"unitEndLabName\":\"码\",\"regName\":\"速度|布长|鞋码\",\"icon\":\"\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"ML\",\"regName\":\"容积\",\"icon\":\"\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"%\",\"regName\":\"率\",\"icon\":\"\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"\",\"regName\":\"备注\",\"icon\":\"icon-book-open\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"\",\"regName\":\"邮件|email|电子邮件\",\"icon\":\"icon-envelope\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"\",\"regName\":\"电话|手机|联系方式\",\"icon\":\"icon-envelope\",\"tip\":\"\"}," +
			"{\"unitEndLabName\":\"\",\"regName\":\"网站|域名|链接|主页|博客\",\"icon\":\"icon-link\",\"tip\":\"http://ccflow.org\"}]";
	/**
	 * 根据中文名称返回字段类型
	 * @param arrayListName
	 * @return
	 */
	public static String GenerDataTypeByCHName(List<String> arrayListName) throws Exception {

		MapAttrs mapAttrs = new MapAttrs();
		String result = String.join(",", arrayListName);
		DataTable dt = new DataTable();
		if (!DataType.IsNullOrEmpty(result)) {
			QueryObject qo = new QueryObject(mapAttrs);
			qo.AddWhereIn("Name", "(" + bp.port.Glo.GenerWhereInSQL(String.join(",", result)) + ")");
			dt = qo.DoQueryToTable();
		}

		DataTable dtFieldNameKeys = Json.ToDataTable(fieldNameKeys);
		...
		try
		{
			for (int i = 0; i < arrayListName.size(); i++)
			{
				...
				DataRow[] datas = dt.Select("Name='" + name + "'");
				if (datas.length > 0)
				{
					Hashtable hashtable = new Hashtable();
					hashtable.put("name", name);
					hashtable.put("dataType", datas[0].getValue("MyDataType").toString());
					list.add(hashtable);
				}
				else {
					for (DataRow drKey : dtFieldNameKeys.Rows)
					{
						String regex = "";
						try
						{
							regex = drKey.getValue("regName").toString();
						}
						catch (Exception ex)
						{
						...
						Matcher matcher = pattern.matcher(arrayListName.get(i));
						if (matcher.find())
						{
							Hashtable<String, Object> hashtable = new Hashtable<>();
							hashtable.put("name", name);
							hashtable.put("dataType", drKey.getValue("dataType"));
							list.add(hashtable);
							break;
						}
					}
	 ...
	 * @return
	 */
	public static String GenerFildEndInfoByCHName(ArrayList<String> arrayListName) throws Exception {

		DataTable dtFieldEndLabs = Json.ToDataTable(fieldEndLabs);
		...
		try
		{
			for (int i = 0; i < arrayListName.size(); i++)
			{
				...
				for (DataRow dr : dtFieldEndLabs.Rows)
				{
					String regex = "";
					try
					{
						regex = dr.getValue("regName").toString();
					}
					catch (Exception ex)
					{
						Log.DebugWriteError(ex);
					...
					if (matcher.find())
					{
						Hashtable<String, Object> hashtable = new Hashtable<>();
						hashtable.put("name", name);
						hashtable.put("unitEndLabName", dr.getValue("unitEndLabName"));
						hashtable.put("icon", dr.getValue("icon"));
						hashtable.put("tip", dr.getValue("tip"));
						list.add(hashtable);
						break;
					}
				}
	...
	public static void SaveFieldEnum(String fk_mapdata, String fieldName, String fieldDesc, String enumKey, UIContralType ctrlType, float x, float y, int colSpan) throws Exception {
		...
		if (ma.getUIContralType() == UIContralType.RadioBtn)
		{
			...
			for (SysEnum item : ses.ToJavaList())
			{
				...
				if(DataType.IsNullOrEmpty(item.getStrKey())==false)
					rb.setIntKey(item.getStrKey());
				else
					rb.setIntKey(String.valueOf(item.getIntKey()));
	...
	*/
	...
	/**
	 ...
	 @param md
	*/
	public static void RepareCCForm(MapData md) throws Exception {

		if (md.getEntityType() == EntityType.FrmEntityNoName || md.getEntityType() == EntityType.EntityTree)
			return;
	...
	*/
	...
	public static DataSet GenerHisDataSet(String frmID, String frmName, MapData md) throws Exception {
		...
		MapData mapData = (md != null) ? md : new MapData(frmID);
		if (DataType.IsNullOrEmpty(mapData.getName()) && !DataType.IsNullOrEmpty(frmName)) {
			mapData.setName(frmName);
		}
		List<Callable<DataTable>> tasks = new ArrayList<>();
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> mapData.ToDataTableField("Sys_MapData"), "Sys_MapData"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> mapData.getGroupFields().ToDataTableField("Sys_GroupField"), "Sys_GroupField"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getOrigMapDtls()).ToDataTableField("Sys_MapDtl"), "Sys_MapDtl"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> mapData.getSysEnums().ToDataTableField("Sys_Enum"), "Sys_Enum"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> mapData.getMapAttrs().ToDataTableField("Sys_MapAttr"), "Sys_MapAttr"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getMapExts()).ToDataTableField("Sys_MapExt"), "Sys_MapExt"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getFrmRBs()).ToDataTableField("Sys_FrmRB"), "Sys_FrmRB"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getFrmImgs()).ToDataTableField("Sys_FrmImg"), "Sys_FrmImg"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getMapFrames()).ToDataTableField("Sys_MapFrame"), "Sys_MapFrame"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getFrmAttachments()).ToDataTableField("Sys_FrmAttachment"), "Sys_FrmAttachment"));
		tasks.add(AsyncTaskHelper.createTrackedTask(() -> Objects.requireNonNull(mapData.getFrmImgAths()).ToDataTableField("Sys_FrmImgAth"), "Sys_FrmImgAth"));
		long totalStart = System.currentTimeMillis();
		List<Future<DataTable>> futures = null;
		boolean hasTimeoutOrCancellation = false;
		try {
			futures = executor.invokeAll(tasks, 5, TimeUnit.SECONDS); // 使用配置的超时时间
			hasTimeoutOrCancellation = AsyncTaskHelper.isHasTimeoutOrCancellation(ds, futures, hasTimeoutOrCancellation, logger);
			if (hasTimeoutOrCancellation) {
				logger.warn("加载表单数据因超时或取消而失败。{}", JflowThreadPoolTaskExecutor.getPoolStatus());
				throw new TimeoutException("加载表单数据超时或被取消，部分任务未能在规定时间内完成。");
			}
			logger.info("GenerHisDataSet接口完成, 总耗时: {} ms", System.currentTimeMillis() - totalStart);
			return ds;
		} catch (InterruptedException ie) {
			Thread.currentThread().interrupt();
			logger.warn("加载表单数据的 invokeAll 操作被中断", ie);
			if (futures != null) { for(Future<?> f : futures) { f.cancel(true); } }
			throw new Exception("加载表单数据操作被中断。", ie);
		} catch (RejectedExecutionException ree) {
			logger.warn("任务提交被拒绝！线程池已满: {}", JflowThreadPoolTaskExecutor.getPoolStatus(), ree);
			throw new Exception("系统当前繁忙，请稍后重试。", ree);
		} catch (Exception e) {
			if (!(e instanceof TimeoutException)) {
				logger.error("处理 GenerHisDataSet 时发生未预期错误", e);
			}
			throw e;
		}
	}
