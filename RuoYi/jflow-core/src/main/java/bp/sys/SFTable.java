...
public class SFTable extends EntityNoName
{
	...
	*/
	...
	public boolean getIsExits() throws Exception {
		...
		if(this.getNo().startsWith(WebUser.getOrgNo() + "_")==false)
	...
	*/
	private void checkJsonField(JSONObject jsonItem, String fieldNo, String fieldName, String fieldParentNo) throws Exception {
		...
		try {
			...
			// 判断是否包含所匹配的项
			if (jsonItem.containsKey("No") == false)
				if (!DataType.IsNullOrEmpty(fieldNo) && jsonItem.containsKey(fieldNo) == false)
					throw new Exception("字典[" + this.getNo() + "]API不包含定义的No列：" + fieldNo + ",请确认列明标准转换是否正确?");

			if (jsonItem.containsKey("Name") == false)
				if (!DataType.IsNullOrEmpty(fieldName) && jsonItem.containsKey(fieldName) == false)
					throw new Exception("字典[" + this.getNo() + "]API不包含定义的Name列：" + fieldName + ",请确认列明标准转换是否正确?");

			if (this.getCodeStruct() == CodeStruct.Tree) {
				if (jsonItem.containsKey("ParentNo") == false)
					if (!DataType.IsNullOrEmpty(fieldParentNo) && jsonItem.containsKey(fieldParentNo) == false)
						throw new Exception("字典[" + this.getNo() + "]API不包含定义的ParentNo列：" + fieldParentNo + ",请确认列明标准转换是否正确?");
			}
		}catch (Exception ex)
		{
			String helpInfo = ",在字典属性里找到[返回数据&列对应],并仔细查看说明.";
			throw new Exception(ex.getMessage() + " - " + helpInfo + " \t\n 原始数据:"+ jsonItem.toString());
	...
	*/
	private JSONObject GetJSONByTargetName(JSONObject source, String fieldNo, String fieldName, String fieldParentNo) throws Exception {
		...
		if (!DataType.IsNullOrEmpty(fieldNo))
		{
			...
			if (source.containsKey("No") == true)
				nObj.put("No", source.get("No"));
			else
		...
		{
			if (source.containsKey("Name") == true)
				nObj.put("Name", source.get("Name"));
			else
		...
		if (this.getCodeStruct() == CodeStruct.Tree)
		{
			...
			{
				if (source.containsKey("ParentNo") == true)
					nObj.put("ParentNo", source.get("ParentNo"));
				else
	...
	public final String getRemark()  {
		...
		return this.GetValStringByKey("Remark");
	}
	...
	public final String getJsonNode()
	{
		return this.GetValStringByKey("JsonNode");
	}
	...
	*/
	...
	public final DataTable GenerHisDataTable(Hashtable ht) throws Exception {
		//处理参数.
		ht = OverrideFile.DealHT_ParamAlia(ht,this.getParamAlia());
		...
		if (Objects.equals(this.getSrcType(), DictSrcType.WebServices))
		{
			...
			for (String p : ps)
			{
				...
				try
				{
					...
					if (pa[1].contains("@WebUser.DeptNo"))
					{
						pa[1] = pa[1].replace("@WebUser.DeptNo", WebUser.getDeptNo());
					}
				}
		...
		{
			ResultObj objWeb= bp.en.OverrideFile.Data_WebApi(ht, this.getRequestMethod(), this.getSFDBSrcNo(),
					this.getSelectStatement(), this.getParamAlia(), this.getPostDoc(),this.getHeaderDoc());
			if (objWeb.code != 200)
				throw new Exception("err@错误:" + objWeb.message + " data:" + objWeb.data);
			...
			ObjectMapper objectMapper = new ObjectMapper();
			JsonNode jsonNodeO  = objectMapper.readTree(objWeb.data);

			// 如果是JSON数组
			if (jsonNodeO.isArray())
			{
				// 新的对象，用来删除原对象无用字段
				JSONArray arr = JSONArray.fromObject(objWeb.data);
			...
			// 如果是JSON对象
			if (jsonNodeO.isObject())
			{
				JSONObject jsonItem =JSONObject.fromObject(objWeb.ToSpecString());
				// 判断是不是有根节点
				// 如果有
				...
				JSONObject currObj = JSONObject.fromObject(objWeb.ToSpecString());
			...
			throw new RuntimeException("Web_API 没有正确返回JSON字符串："+ objWeb.ToSpecString());
		}
		///#endregion
		...
		if (Objects.equals(this.getSrcType(), DictSrcType.SQL))
		{
			...
			{
				String remark=this.GetValStringByKey("Remark");
				if(DataType.IsNullOrEmpty(remark)==false){
		...
		if (Objects.equals(this.getSrcType(), DictSrcType.SysDict) || Objects.equals(this.getSrcType(), DictSrcType.CreateTable))
		{
			...
			{
				sql = "SELECT BH AS No, Name FROM Sys_SFTableDtl WHERE FK_SFTable='" + this.getNo() + "'";
			}
			else
			{
				sql = "SELECT BH AS No, Name,ParentNo FROM Sys_SFTableDtl WHERE FK_SFTable='" + this.getNo() + "'";
			}
			DataTable dt = src.RunSQLReturnTable(sql);
	...
	public String SaveData(String no, String name, String fk_SFTable) throws Exception {
		return InsertData(no, name, fk_SFTable);
	}
	/**
	 修改外键数据
	...
	*/
	...
	/**
	 ...
	 @return
	*/
	public final String InsertData(String no, String name, String fk_SFTable) throws Exception {

		if (Objects.equals(this.getSrcType(), bp.sys.DictSrcType.SysDict))
		{
			String mypk = fk_SFTable + "_" + no;
			SFTableDtl dtl = new SFTableDtl();
			dtl.setMyPK(mypk);
		...
		{
			String sql = "SELECT No,Name FROM " + fk_SFTable + " WHERE No='" + no + "'";
			DataTable dt = DBAccess.RunSQLReturnTable(sql);
			if (dt.Rows.size() == 0)
				sql = "insert into  " + fk_SFTable + "(No,Name) values('" + this.getNo() + "','" + this.getName() + "')";
			else
				sql = "UPDATE " + fk_SFTable + " SET Name = '" + this.getName() + "' WHERE No = '" + this.getNo() + "'";
			DBAccess.RunSQL(sql);
			return "执行成功.";
	...
	*/
	public final String getSrcType() throws Exception {
		...
		{
			String src = this.GetValStringByKey(SFTableAttr.DBSrcType);
	...
	public final void setSrcType(String value){
		this.SetValByKey(SFTableAttr.DBSrcType, value);
	}
	/**
	 数据源类型名称
	*/
	...
	*/
	...
	public Map getEnMap() {
		...
		map.AddTBInt("IsPara", 0, "参数个数", true, true);
		...
		map.AddTBString(SFTableAttr.SelectStatement, null, "查询语句/Url", true, false, 0, 1000, 600, true);

		map.AddTBString("PostDoc", null, "POST内容", false, false, 0, 200, 20);
		map.AddTBString("HeaderDoc", null, "Header内容", false, false, 0, 200, 20);
		map.AddTBString("ParamAlia", null, "参数别名", false, false, 0, 200, 20);
		...
		map.AddTBString("TestParas", null, "测试参数", false, false, 0, 300, 20);//@hongyan.
	...
	*/
	...
	////参数个数.
	//if (this.SelectStatement.Contains("@Key") == true)
	//    this.SetValByKey("IsPara", 1);
	//else
	//    this.SetValByKey("IsPara", 0);
	...
	protected void afterInsert() throws Exception
	{
		...
		///#region 执行方法.
	public String TS_YuanShi_Data_WebApi_Test() throws Exception {
		Hashtable ht = new Hashtable();

		String paras = this.GetValStrByKey("TestParas");
		AtPara para = new AtPara(paras);

		ResultObj obj = bp.en.OverrideFile.Data_WebApi((para.getHisHT()), this.getRequestMethod(), this.getSFDBSrcNo(),
				this.getSelectStatement(), this.getParamAlia(), this.getPostDoc(), this.getHeaderDoc());
	...
	/// </summary>
	/// <returns></returns>
	public String TS_Stuct_Data_WebApi() throws Exception {
		try
		{
			String paras = this.GetValStrByKey("TestParas");
			return this.GenerJsonByPara(paras);
		}
		catch (Exception ex)
		{
	...
	public Hashtable TurnToParaHT(String paraString)
	{
		Hashtable ht = new Hashtable();
		if (DataType.IsNullOrEmpty(paraString) == true)
			return ht;
		...
		paraString = paraString.replace(",", "@");
		paraString = paraString.replace(";", "@");
		if (paraString.contains("@") == false && paraString.contains("=")==false)
			paraString = "@Key=" + paraString;

		if (paraString.contains("@") == false)
			paraString = "@" + paraString;

		AtPara atPara = new AtPara(paraString);

		return atPara.getHisHT();
	}
	/**
	 获得原始的数据
	...
	*/
	public final String TS_YuanShi_Data_WebApi() throws Exception {
		try {
			String paras = this.GetValStrByKey("TestParas");
			AtPara para = new AtPara(paras);
			ResultObj obj = bp.en.OverrideFile.Data_WebApi(para.getHisHT(), this.getRequestMethod(),
					this.getSFDBSrcNo(), this.getSelectStatement(), this.getParamAlia(), this.getPostDoc(),
					this.getHeaderDoc());
	...
	/// <summary>
	/// 用于有参测试:TS.frmUI.SFTableSQL
	/// </summary>
	/// <param name="para"></param>
	/// <returns></returns>
	public String GenerJsonByPara(String para) throws Exception {
		Hashtable ht = TurnToParaHT(para);
		return GenerDataOfJsonExt(ht);
	}
	/**
	 返回json.
	...
	*/
	public final String GenerDataOfJson() throws Exception {
		return GenerDataOfJsonExt(null);
	}
	...
	/**
	 ...
	 @return
	 */
	public final String GenerDataOfJsonExt(Hashtable htParas) throws Exception {
		try
		{
			DataTable dt = this.GenerHisDataTable(htParas);
