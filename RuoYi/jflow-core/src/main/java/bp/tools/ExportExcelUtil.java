...
public class ExportExcelUtil
{
	...
	public static String ExportDGToExcel(DataSet ds, String title, String paras) throws Exception {
		...
		try
		{
			...
			{
				strLine = strLine + dr.getValue("IDX") + tab;
				for (DataRow attr : AttrsOfGroup.Rows)
				{
					strLine = strLine + dr.getValue(attr.get("KeyOfEn") + "T") + tab;

				}
				for (DataRow attr : AttrsOfNum.Rows)
				{

					strLine = strLine + dr.getValue(attr.get("KeyOfEn").toString()) + tab;
				}
		...
		try
		{
			...
			for (DataRow dr : dt.Rows)
			{
				...
				for (Attr attr : selectedAttrs)
				{
					...
					{
						strLine += "<td>" + (dr.getValue(attr.getKey()).equals(1) ? "是" : "否") + tab + "</td>";
					}
					else
					{
						String text = "";
						...
						{
							text = dr.getValue(attr.getKey() + "Text").toString();
						}
						else if (dt.Columns.contains(attr.getKey() + "T") == true)
						{
							text = dr.getValue(attr.getKey() + "T").toString();
						}
						else
						{
							text = dr.getValue(attr.getKey()).toString();
						}
	...
	public static void ExportDGToExcel(String filePath, Entities ens) throws Exception {

		Entity entity = (Entity)ens.get(0);
		Attrs attrs = entity.getEnMap().getAttrs();
		FileInputStream stream = null;
		Attrs selattrs=new Attrs();
		try
		{
			stream = new FileInputStream(filePath);
			Workbook wb =  WorkbookFactory.create(stream);
			Sheet xssfSheet = wb.getSheetAt(0);
			xssfSheet.protectSheet("123456");//设置内容保护（只读）

			// 设置打印模式
			PrintSetup printSetup = xssfSheet.getPrintSetup();
			printSetup.setPaperSize(PrintSetup.A3_PAPERSIZE); // 设置纸张大小为A4
			printSetup.setLandscape(true); // 设置横向打印
			printSetup.setFitWidth((short)1); // 设置打印宽度适应纸张
			printSetup.setFitHeight((short)0); // 设置打印高度不适应纸张
			...
			for(Row row : xssfSheet){
				...
				if(index== rowSize){
					for(Cell cell : row){
						...
						Attr attr = attrs.GetAttrByKeyOfEn(value.replace("<","").replace(">",""));
						if(attr!=null)
							selattrs.Add(attr);
					}
				...
				for(Cell cell : row){
					String value = cell.getStringCellValue();
					if(value.indexOf("<") !=-1){
						cell.setCellValue(replaceString(value,entity));
					}
				}
			}
			// 设置标题（实现打印时每一页都有同个头部标题）
			int lastRow = rowSize-2;
			int lastCell = xssfSheet.getRow(lastRow).getLastCellNum()-1;
			xssfSheet.setRepeatingRows(new CellRangeAddress(0,lastRow,0,lastCell));
			...
			for(Entity en : ens.ToJavaListEn()){
				Row row = xssfSheet.createRow(rowSize-1);
				int celIdx = 0;
				for(Attr attr : selattrs){
					Cell cell = row.createCell(celIdx);
					cell.setCellStyle(style);
					celIdx++;
					if(attr.getMyDataType() == DataType.AppInt){
						cell.setCellValue(en.GetValIntByKey(attr.getKey()));
						continue;
					}
					if(attr.getMyDataType() == DataType.AppDouble || attr.getMyDataType() == DataType.AppFloat  || attr.getMyDataType() == DataType.AppMoney){
						cell.setCellValue(en.GetValDoubleByKey(attr.getKey()));
						continue;
					}
					if(attr.getMyDataType() == DataType.AppBoolean){
						int val = en.GetValIntByKey(attr.getKey());
						if(val==0) cell.setCellValue("否");
						else cell.setCellValue("是");
						continue;
					}
					if(attr.getItIsEnum()){
						cell.setCellValue(en.GetValDoubleByKey(attr.getKey()+"Text"));
						continue;
					}
					String val = en.GetValStringByKey(attr.getKey()+"Text");
					if(DataType.IsNullOrEmpty(val))
						val = en.GetValStringByKey(attr.getKey()+"T");
					if(DataType.IsNullOrEmpty(val))
						val = en.GetValStringByKey(attr.getKey());
	...
	private static String replaceString(String str,Entity en){
		Pattern p = Pattern.compile("(<[^>]*>)");
		Matcher m = p.matcher(str);
		while(m.find()){
			String val = m.group().replace("<","").replace(">","");
			str = str.replace("<"+val+">",en.GetValStringByKey(val));
		}
		return str;
	}
}
