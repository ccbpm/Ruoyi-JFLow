...
public class GenerBill extends Entity
{
	...
	*/
	...
	public final void setCurrIdx(int value) throws Exception {
		SetValByKey(GenerBillAttr.CurrIdx, value);
	}
	public final String getCurrCheckerNos()  {
		return this.GetValStrByKey(GenerBillAttr.CurrCheckerNos);
	}
	public final void setCurrCheckerNos(String value) throws Exception {
		SetValByKey(GenerBillAttr.CurrCheckerNos, value);
	}
	public final String getCurrCheckerNames()  {
		return this.GetValStrByKey(GenerBillAttr.CurrCheckerNames);
	}
	public final void setCurrCheckerNames(String value) throws Exception {
	...
	*/
	...
	public final void setIdxName(String value)  {
	...
	*/
	public final int getIdx() {
		return this.GetValIntByKey(GenerBillAttr.Idx);
	}
	public final void setIdx(int value) throws Exception {
		SetValByKey(GenerBillAttr.Idx, value);
	}
	...
	*/
	...
	public final DataTable GenerToolbar()
{
	///#region 单据功能.
	...
	dt.Columns.Add("IsEnable"); //是否可用?

	DataRow dr = dt.NewRow();

	if (this.getBillState() == BillState.Draft || this.getBillState() == BillState.Blank || this.getBillState() == BillState.Editing)
	{
		dr.setValue("No", "Save");
		dr.setValue("Name", "保存表单");
		dr.setValue("Icon", "Save");
		dr.setValue("GroupID", "MyBill");
		dr.setValue("IsEnable", "1");
		dt.Rows.add(dr);

		dr = dt.NewRow();
		dr.setValue("No", "SaveAndFrmOver");
		dr.setValue("Name", "保存并归档");
		dr.setValue("Icon", "Save");
		dr.setValue("GroupID", "MyBill");
		dr.setValue("IsEnable", "1");
		dt.Rows.add(dr);
		...
		dr = dt.NewRow();
		dr.setValue("No", "FrmOver");
		dr.setValue("Name", "归档");
		dr.setValue("Icon", "New");
		dr.setValue("GroupID", "MyBill");
		dr.setValue("IsEnable", "1");
		dt.Rows.add(dr);
	...
	//归档
	if (this.getBillState() == BillState.FrmOver)
	{
		dr = dt.NewRow();
		dr.setValue("No", "ArchiveUn");
		dr.setValue("Name", "撤销归档");
		dr.setValue("Icon", "Redo");
		dr.setValue("GroupID", "MyBill");
		dt.Rows.add(dr);
	}

	dr = dt.NewRow();
	dr.setValue("No", "MyBill");
	dr.setValue("Name", "原始表单");
	dr.setValue("Icon", "Call");
	dr.setValue("GroupID", "MyBill");
	dt.Rows.add(dr);
	///#endregion 单据功能.
	...
	@Override
		protected  boolean beforeUpdateInsertAction() throws Exception {
			this.SetValByKey(GenerBillAttr.SendDT, bp.da.DataType.getCurrentDateTime());
