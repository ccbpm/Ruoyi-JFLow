...
public class Log {
	...
	public static void DebugWriteError(Exception ex) {
		DefaultLogWriteLine(LogType.Error, getStackTraceInfo(ex));
	}

	public static String getStackTraceInfo(Exception e) {
		try (StringWriter sw = new StringWriter();
			 PrintWriter pw = new PrintWriter(sw)) {
			e.printStackTrace(pw); // 将异常的栈信息输出到 PrintWriter 中
			return sw.toString();
		} catch (Exception ex) {
			return "发生错误：无法获取堆栈信息";
		}
	}
	...
	public static void DebugWriteException(Exception ex)
	{
		if (logger != null)
		{
